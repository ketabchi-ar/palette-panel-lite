...
class Serene_Panel_Assets {
    ...
    public function enqueue_frontend_assets() {
        ...
        wp_localize_script('serene-panel-js', 'sereneConfig', [
            ...
            'lucky_wheel_slices'  => class_exists('Palette_Panel_Lucky_Wheel') ? Palette_Panel_Lucky_Wheel::get_wheel_slices() : [],
            'can_spin_wheel'      => ($current_uid && class_exists('Palette_Panel_Lucky_Wheel')) ? Palette_Panel_Lucky_Wheel::can_user_spin($current_uid) : false,
            'auth_options'        => [
    ...
    public function enqueue_admin_assets($hook) {
        $is_palette_page = (strpos($hook, 'palette-panel') !== false || strpos($hook, 'serene-panel') !== false || strpos($hook, 'palette-sms') !== false || strpos($hook, 'palette-tickets') !== false);
        if (!$is_palette_page && isset($_GET['page']) && (strpos($_GET['page'], 'palette') !== false || strpos($_GET['page'], 'serene') !== false)) {
            $is_palette_page = true;
        }
        if (!$is_palette_page) return;
        ...
        wp_enqueue_style('serene-material-symbols-cdn', 'https://cdn.jsdelivr.net/npm/material-symbols@0.14.3/index.min.css');
