<?php
if (!function_exists('palette_render_pro_lock_card')) {
    function palette_render_pro_lock_card($title, $desc) {
        ?>
        <div class="p-10 bg-slate-900/60 rounded-3xl border border-white/10 text-center relative overflow-hidden backdrop-blur-md my-4 shadow-2xl">
            <div class="w-16 h-16 rounded-2xl bg-gradient-to-br from-amber-400 to-amber-600 text-white flex items-center justify-center text-3xl mx-auto mb-4 shadow-lg shadow-amber-500/20">
                <span class="material-symbols-outlined">lock</span>
            </div>
            <span class="inline-block px-3.5 py-1 rounded-full bg-amber-500/10 text-amber-400 text-[11px] font-black border border-amber-500/20 mb-3 tracking-wide">
                قابلیت ویژه نسخه حرفه‌ای (PRO)
            </span>
            <h3 class="text-base font-black text-white mb-2"><?php echo esc_html($title); ?></h3>
            <p class="text-xs text-slate-300 max-w-lg mx-auto leading-relaxed mb-6"><?php echo esc_html($desc); ?></p>
            <a href="https://www.zhaket.com" target="_blank" class="inline-flex items-center gap-2 px-7 py-3.5 bg-gradient-to-r from-amber-500 via-amber-600 to-amber-700 hover:from-amber-600 hover:to-amber-800 text-white text-xs font-black rounded-2xl shadow-xl shadow-amber-600/25 transition-all transform hover:-translate-y-0.5">
                <span>مشاهده و دریافت نسخه کامل از ژاکت</span>
...
$theme_colors = class_exists('Palette_Panel_Assets') ? Palette_Panel_Assets::get_theme_colors() : (class_exists('Serene_Panel_Assets') ? Serene_Panel_Assets::get_theme_colors() : []);
$menu_items = class_exists('Palette_Panel_Menu_Builder') ? Palette_Panel_Menu_Builder::get_menu_items() : (class_exists('Serene_Panel_Menu_Builder') ? Serene_Panel_Menu_Builder::get_menu_items() : []);
$custom_fields = class_exists('Palette_Panel_Form_Builder') ? Palette_Panel_Form_Builder::get_custom_fields() : (class_exists('Serene_Panel_Form_Builder') ? Serene_Panel_Form_Builder::get_custom_fields() : []);
$wheel_slices = class_exists('Palette_Panel_Lucky_Wheel') ? Palette_Panel_Lucky_Wheel::get_wheel_slices() : (class_exists('Serene_Panel_Lucky_Wheel') ? Serene_Panel_Lucky_Wheel::get_wheel_slices() : []);
$loyalty_tiers = class_exists('Palette_Panel_Loyalty_Tiers') ? Palette_Panel_Loyalty_Tiers::get_tiers() : [];
$is_woo_wallet = class_exists('Palette_Panel_Wallet') ? Palette_Panel_Wallet::is_woo_wallet_active() : (class_exists('Serene_Panel_Wallet') ? Serene_Panel_Wallet::is_woo_wallet_active() : false);
$carriers = class_exists('Palette_Panel_Post_Tracker') ? Palette_Panel_Post_Tracker::get_carriers() : (class_exists('Serene_Panel_Post_Tracker') ? Serene_Panel_Post_Tracker::get_carriers() : []);
$ticket_departments = class_exists('Palette_Panel_Tickets') ? Palette_Panel_Tickets::get_departments() : (class_exists('Serene_Panel_Tickets') ? Serene_Panel_Tickets::get_departments() : []);

// Fallback Asset Injection
if (!wp_script_is('serene-tailwind', 'done')) {
    $colors = class_exists('Palette_Panel_Assets') ? Palette_Panel_Assets::get_theme_colors() : [];
    ?>
    ...
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/material-symbols@0.14.3/index.min.css">
    <link rel="stylesheet" href="<?php echo esc_url(PALETTE_PANEL_ASSETS_URL . 'css/serene-panel.css'); ?>">
    ...
    tailwind.config = {
        ...
        theme: {
            extend: {
                colors: {
                    'primary': '<?php echo esc_js($colors['primary'] ?? '#4c5e8b'); ?>',
                    'primary-dim': '<?php echo esc_js($colors['primary_dim'] ?? '#40527f'); ?>',
                    'primary-container': '<?php echo esc_js($colors['primary_container'] ?? '#b6c8fc'); ?>',
                    'secondary': '<?php echo esc_js($colors['secondary'] ?? '#585f72'); ?>',
                    'surface': '<?php echo esc_js($colors['surface'] ?? '#f9f9fe'); ?>',
                    'surface-container': '<?php echo esc_js($colors['surface_container'] ?? '#ecedf6'); ?>',
                    'on-surface': '<?php echo esc_js($colors['on_surface'] ?? '#2f323b'); ?>'
                }
            }
        }
    };
    ...
    <div class="max-w-7xl mx-auto py-6 space-y-6 pb-28">
        ...
        <form id="serene-settings-form">
                ...
                <div id="sub-auth-methods" class="sub-content hidden space-y-6">
                    <div class="bg-surface-container-lowest p-7 rounded-[2rem] border border-outline-variant/30 shadow-sm space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <!-- Biometrics Passkeys (Locked in Lite) -->
                            <div class="p-5 bg-amber-500/5 rounded-2xl space-y-3 border border-amber-500/30 relative overflow-hidden">
                                <div class="flex items-center justify-between">
                                    <span class="text-xs font-bold text-on-surface flex items-center gap-2">
                                        <span class="material-symbols-outlined text-amber-600">fingerprint</span>
                                        <span>ورود بیومتریک و اثر انگشت (Passkeys)</span>
                                    </span>
                                    <span class="px-2.5 py-0.5 rounded-full bg-amber-500/20 text-amber-700 text-[10px] font-black border border-amber-500/30">ویژه نسخه PRO</span>
                                </div>
                                <p class="text-[11px] text-slate-600">ورود بدون نیاز به رمز و پیامک با اثر انگشت یا FaceID گوشی (WebAuthn / FIDO2).</p>
                                <div class="pt-1">
                                    <a href="https://www.zhaket.com" target="_blank" class="inline-flex items-center gap-1.5 text-amber-700 hover:text-amber-800 font-black text-[11px]">
                                        <span>فعال‌سازی در نسخه پرو ژاکت</span>
            ...
            <div id="cat-finance" class="main-cat-content hidden space-y-6">
                ...
                <!-- Sub-tab 2.1: Card-to-Card Gateway (Locked in Lite) -->
                <div id="sub-c2c" class="sub-content space-y-6">
                    <?php palette_render_pro_lock_card('درگاه اختصاصی کارت‌به‌کارت و وب‌هوک بانکی', 'سیستم احراز هوشمند فیش واریزی، تایید آنی سفارش، وب‌هوک رله پیامک و اتصال به اپلیکیشن اندروید در نسخه حرفه‌ای ژاکت فعال است.'); ?>
                </div>
                ...
                <!-- Sub-tab 2.3: Bulk Wallet Adjustment (Locked in Lite) -->
                <div id="sub-wallet-bulk" class="sub-content hidden space-y-6">
                    <?php palette_render_pro_lock_card('شارژ و کسر گروهی موجودی کیف پول + پیامک اطلاع‌رسانی', 'امکان ارسال پیامک واریز و کسر و شارژ دسته‌ای تمام کاربران در نسخه حرفه‌ای ژاکت فعال است.'); ?>
                </div>
                ...
                <div id="sub-form" class="sub-content hidden space-y-6">
                    <div class="bg-surface-container-lowest p-7 rounded-[2rem] border border-outline-variant/30 shadow-sm space-y-6">
                        <div class="flex justify-between items-center flex-wrap gap-4 border-b border-outline-variant/20 pb-5">
                            ...
                            <div class="inline-flex items-center gap-2 px-4 py-2 bg-amber-500/10 border border-amber-500/30 rounded-2xl text-[11px] font-bold text-amber-700">
                                <span class="material-symbols-outlined text-sm">lock</span>
                                <span>افزودن فیلدهای اختصاصی نامحدود در نسخه پرو فعال است</span>
                                <a href="https://www.zhaket.com" target="_blank" class="underline font-black mr-1 text-amber-800">خرید از ژاکت</a>
                            </div>
                        </div>
            ...
            <div id="cat-engagement" class="main-cat-content hidden space-y-6">
                ...
                <!-- Sub-tab 4.1: Lucky Wheel (Locked in Lite) -->
                <div id="sub-wheel" class="sub-content space-y-6">
                    <?php palette_render_pro_lock_card('مدیریت گردونه شانس، جوایز و کدهای تخفیف پویا', 'سیستم گیمیفیکیشن، گردونه شانس با شانس‌های متغیر، کدهای تخفیف خودکار و شارژ کیف پول در نسخه حرفه‌ای ژاکت فعال است.'); ?>
                </div>
            ...
            <div id="cat-advanced" class="main-cat-content hidden space-y-6">
                ...
                <!-- 3. Dynamic Loyalty Tiers Sub-tab (Locked in Lite) -->
                <div id="sub-feat-loyalty" class="sub-content hidden space-y-6">
                    <?php palette_render_pro_lock_card('باشگاه مشتریان و سطوح وفاداری پویا (Loyalty Tiers)', 'تعریف نامحدود رده‌های کاربری بر اساس میانگین خرید، پاداش کش‌بک درصدی خودکار و نشان‌های VIP در نسخه حرفه‌ای ژاکت فعال است.'); ?>
                </div>

                <!-- 4. Price & Stock Alerts Sub-tab (Locked in Lite) -->
                <div id="sub-feat-price-alerts" class="sub-content hidden space-y-6">
                    <?php palette_render_pro_lock_card('سیستم هشدار قیمت و موجودی کالا (Price Alerts)', 'اطلاع‌رسانی پیامکی خودکار به کاربران هنگام تخفیف یا موجود شدن کالاهای موردعلاقه در نسخه حرفه‌ای ژاکت فعال است.'); ?>
                </div>

                <!-- 5. Affiliate Sub-tab (Locked in Lite) -->
                <div id="sub-feat-affiliate" class="sub-content hidden space-y-6">
                    <?php palette_render_pro_lock_card('سیستم همکاری در فروش و بازاریابی (Affiliate)', 'لینک‌های اختصاصی بازاریابی، محاسبه هوشمند کمیسیون و تسویه خودکار با کیف پول در نسخه حرفه‌ای ژاکت فعال است.'); ?>
                </div>
                ...
                <!-- 7. Shahkar Sub-tab (Locked in Lite) -->
                <div id="sub-feat-shahkar" class="sub-content hidden space-y-6">
                    <?php palette_render_pro_lock_card('احراز هویت و تطبیق کدملی سامانه شاهکار (Shahkar OTP)', 'استعلام رسمی و تطبیق هویت کدملی با شماره موبایل کاربر جهت احراز هویت قانونی در نسخه حرفه‌ای ژاکت فعال است.'); ?>
                </div>

                <!-- 8. PWA Sub-tab (Locked in Lite) -->
                <div id="sub-feat-pwa" class="sub-content hidden space-y-6">
                    <?php palette_render_pro_lock_card('وب‌اپلیکیشن پیش‌رونده (PWA)', 'امکان نصب اختصاصی وب‌اپلیکیشن روی صفحه اصلی موبایل (iOS و اندروید) با آیکون و اسپلش‌اسکرین اختصاصی در نسخه حرفه‌ای ژاکت فعال است.'); ?>
                </div>
