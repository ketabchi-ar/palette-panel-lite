...
<nav class="md:hidden fixed bottom-0 left-0 right-0 bg-surface-container-lowest/95 backdrop-blur-lg border-t border-outline-variant/30 z-30 px-2 py-1.5 flex justify-around items-center shadow-lg">
    ...
    <a href="?tab=tickets" class="flex flex-col items-center gap-0.5 py-1 px-2 rounded-xl text-[10px] font-bold <?php echo $current_tab === 'tickets' ? 'text-primary' : 'text-on-surface-variant'; ?>">
        <span class="material-symbols-outlined text-xl" <?php echo $current_tab === 'tickets' ? 'style="font-variation-settings: \'FILL\' 1;"' : ''; ?>>chat</span>
        <span>تیکت‌ها</span>
    </a>
